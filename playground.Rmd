...
```{r}
# By looking at the above raster and inspecting the raster's resolution, something does not add up - the resolution of every pixel is way too large
res(aarhus_veg) # this is not accurate
crs(aarhus_veg) # verify that the units are in meters

# Measuring the whole raster from top to bottom on Google Maps gives us a height of 22,590 meters
raster_height <- 22590 # meters

# Everything (including the original resolution and hand measuring shows that the pixels are square)
pixel_res = raster_height / nrow(aarhus_veg) # ~20 meters

# Overwrite resolution with (more) correct resolution
res(aarhus_veg) <- pixel_res
```

## Filter off small areas
```{r}
# m^2 for minimum allowed size to be called a 'green area'
area_min_size <- 201 # 10 pixels (200m^2) seems to be just too small, so we add 1 to only select areas of 11 pixels and above
...
for (key in keys(id_counts)) {
  ...
  area_size <- id_counts[[key]] * pixel_res
...
veg_colors <- colorRampPalette(c("darkgreen", "lightblue"))(100)
plot(aarhus_crow_heatmap, col = veg_colors)
